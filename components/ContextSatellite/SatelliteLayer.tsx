...
import { generateImageApi, getModelBySize } from '../../services/api';
...
export const SatelliteLayer: React.FC<SatelliteLayerProps> = ({ onInitGenerations, onUpdateGeneration }) => {
  ...
  const { selectedIds, apiKey } = useSelectionStore(); // Need API Key here
  ...
  const handleGenerate = async (prompt: string, options: any) => {
    if (!apiKey) {
      alert("Please set API Key in settings first!"); // Simple alert for now
      return;
    }
    ...
    placeholderIds.forEach(pid => {
       if (mode === 'IMAGE') {
           const payload = {
              model: getModelBySize('1k'), // Defaulting to 1k for now
              prompt: prompt,
              size: '1k',
